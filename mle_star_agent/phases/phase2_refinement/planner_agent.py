...
def _infer_mechanism_class(target_component: str, strategy_text: str) -> str:
    ...
    inferred = f"inferred_{(target_component or 'unknown').strip().lower()}"
    logger.warning(
        "_infer_mechanism_class: no keyword matched for target_component=%r strategy_text=%r "
        "— bucketing as %r instead of silent 'unknown'.",
        target_component,
        strategy_text,
        inferred,
    )
...
def _context_reports_block(state: dict) -> str:
    ...
    # ADVISORY small-data technique hints retrieved by A_retriever (augmentation,
    # loss, calibration ideas). These are NOT pre-vetted strategies — any strategy
    # the planner derives from them must still pass the
    # KNOWN_FAILED_STRATEGY_FINGERPRINTS gate in save_strategy_candidates_fn.
    hints = state.get("retrieved_technique_hints", []) or []
    if isinstance(hints, list) and hints:
        hint_strs = [str(h).strip() for h in hints if str(h).strip()]
        if hint_strs:
            parts.append(
